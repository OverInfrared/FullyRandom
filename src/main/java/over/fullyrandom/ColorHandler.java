...
public class ColorHandler {
    ...
    public static void registerBlockColors(final ColorHandlerEvent.Block event) {
        for (int i = 0; i < MainConfig.oreAmount.get(); i++) {
            int finalI = i;
            event.getBlockColors().register((state, world, pos, tintIndex) -> { if (tintIndex == 1) { return Randomizer.blockProperties.getColor(finalI); } else { return 0xFFffffff; } }, ModBlocks.oreBlocks[i]);
        }
    }
