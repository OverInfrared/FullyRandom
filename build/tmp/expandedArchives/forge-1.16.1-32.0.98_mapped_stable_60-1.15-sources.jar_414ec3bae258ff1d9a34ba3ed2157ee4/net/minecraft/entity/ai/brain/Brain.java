...
public class Brain<E extends LivingEntity> {
   ...
   public void func_218210_a(ServerWorld worldIn, E entityIn) {
      this.func_233721_k_();
      this.func_233711_c_(worldIn, entityIn);
      this.func_218218_d(worldIn, entityIn);
      this.func_218222_e(worldIn, entityIn);
   }

   private void func_233711_c_(ServerWorld p_233711_1_, E p_233711_2_) {
      for(Sensor<? super E> sensor : this.sensors.values()) {
         sensor.func_220973_b(p_233711_1_, p_233711_2_);
      }
   ...
   public void func_218227_b(ServerWorld worldIn, E owner) {
      long i = owner.world.getGameTime();

      for(Task<? super E> task : this.func_233712_d_()) {
         task.func_220380_e(worldIn, owner, i);
      }

   }

   private void func_218218_d(ServerWorld worldIn, E entityIn) {
            ...
            if (this.activities.contains(activity)) {
               for(Task<? super E> task : entry.getValue()) {
                  if (task.getStatus() == Task.Status.STOPPED) {
                     task.func_220378_b(worldIn, entityIn, i);
                  }
               }
            }
         }
   ...
   private void func_218222_e(ServerWorld worldIn, E entityIn) {
      long i = worldIn.getGameTime();

      for(Task<? super E> task : this.func_233712_d_()) {
         task.func_220377_c(worldIn, entityIn, i);
      }
