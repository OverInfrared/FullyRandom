...
public class FarmTask extends Task<VillagerEntity> {
   ...
   protected boolean func_212832_a_(ServerWorld worldIn, VillagerEntity owner) {
      if (!net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(worldIn, owner)) {
         ...
         for(int i = -1; i <= 1; ++i) {
            for(int j = -1; j <= 1; ++j) {
               for(int k = -1; k <= 1; ++k) {
                  blockpos$mutable.func_189532_c(owner.func_226277_ct_() + (double)i, owner.func_226278_cu_() + (double)j, owner.func_226281_cx_() + (double)k);
   ...
   @Nullable
   private BlockPos func_223517_a(ServerWorld serverWorldIn) {
      return this.farmableBlocks.isEmpty() ? null : this.farmableBlocks.get(serverWorldIn.getRandom().nextInt(this.farmableBlocks.size()));
   }

   private boolean func_223516_a(BlockPos pos, ServerWorld serverWorldIn) {
      BlockState blockstate = serverWorldIn.getBlockState(pos);
      Block block = blockstate.getBlock();
      Block block1 = serverWorldIn.getBlockState(pos.func_177977_b()).getBlock();
   ...
   protected void func_212833_d_(ServerWorld worldIn, VillagerEntity owner, long gameTime) {
      if (this.field_220422_a == null || this.field_220422_a.withinDistance(owner.getPositionVec(), 1.0D)) {
         if (this.field_220422_a != null && gameTime > this.field_220425_d) {
            ...
            Block block = blockstate.getBlock();
            Block block1 = worldIn.getBlockState(this.field_220422_a.func_177977_b()).getBlock();
