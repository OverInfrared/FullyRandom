...
public class WorldBorderCommand {
   ...
   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register(Commands.literal("worldborder").requires((p_198903_0_) -> {
         ...
         return setSize(p_198908_0_.getSource(), p_198908_0_.getSource().func_197023_e().getWorldBorder().getDiameter() + (double)FloatArgumentType.getFloat(p_198908_0_, "distance"), 0L);
      }).then(Commands.argument("time", IntegerArgumentType.integer(0)).executes((p_198901_0_) -> {
         return setSize(p_198901_0_.getSource(), p_198901_0_.getSource().func_197023_e().getWorldBorder().getDiameter() + (double)FloatArgumentType.getFloat(p_198901_0_, "distance"), p_198901_0_.getSource().func_197023_e().getWorldBorder().getTimeUntilTarget() + (long)IntegerArgumentType.getInteger(p_198901_0_, "time") * 1000L);
   ...
   private static int setDamageBuffer(CommandSource source, float distance) throws CommandSyntaxException {
      WorldBorder worldborder = source.func_197023_e().getWorldBorder();
   ...
   private static int getSize(CommandSource source) {
      double d0 = source.func_197023_e().getWorldBorder().getDiameter();
