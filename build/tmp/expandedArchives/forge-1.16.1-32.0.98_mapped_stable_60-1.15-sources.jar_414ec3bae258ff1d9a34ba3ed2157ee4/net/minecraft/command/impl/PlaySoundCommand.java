   ...
   private static int playSound(CommandSource source, Collection<ServerPlayerEntity> targets, ResourceLocation soundIn, SoundCategory category, Vector3d pos, float volume, float pitch, float minVolume) throws CommandSyntaxException {
      ...
      while(true) {
         ...
         while(true) {
            ...
            double d1 = pos.x - serverplayerentity.func_226277_ct_();
            double d2 = pos.y - serverplayerentity.func_226278_cu_();
            double d3 = pos.z - serverplayerentity.func_226281_cx_();
            ...
            if (!(minVolume <= 0.0F)) {
               ...
               vector3d = new Vector3d(serverplayerentity.func_226277_ct_() + d1 / d5 * 2.0D, serverplayerentity.func_226278_cu_() + d2 / d5 * 2.0D, serverplayerentity.func_226281_cx_() + d3 / d5 * 2.0D);
               f = minVolume;
               break;
            }
         }
