...
public class CrossbowItem extends ShootableItem implements IVanishable {
   ...
   private static void fireProjectile(World worldIn, LivingEntity shooter, Hand handIn, ItemStack crossbow, ItemStack projectile, float soundPitch, boolean isCreativeMode, float velocity, float inaccuracy, float projectileAngle) {
      if (!worldIn.isRemote) {
         ...
         if (flag) {
            projectileentity = new FireworkRocketEntity(worldIn, projectile, shooter, shooter.func_226277_ct_(), shooter.func_226280_cw_() - (double)0.15F, shooter.func_226281_cx_(), true);
         } else {
         ...
         worldIn.playSound((PlayerEntity)null, shooter.func_226277_ct_(), shooter.func_226278_cu_(), shooter.func_226281_cx_(), SoundEvents.ITEM_CROSSBOW_SHOOT, SoundCategory.PLAYERS, 1.0F, soundPitch);
      }
   }
