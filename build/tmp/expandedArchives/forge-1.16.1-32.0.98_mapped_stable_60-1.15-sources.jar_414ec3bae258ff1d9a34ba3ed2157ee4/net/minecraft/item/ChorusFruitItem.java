...
public class ChorusFruitItem extends Item {
   ...
   public ItemStack onItemUseFinish(ItemStack stack, World worldIn, LivingEntity entityLiving) {
      ...
      if (!worldIn.isRemote) {
         ...
         for(int i = 0; i < 16; ++i) {
            double d3 = entityLiving.func_226277_ct_() + (entityLiving.getRNG().nextDouble() - 0.5D) * 16.0D;
            double d4 = MathHelper.clamp(entityLiving.func_226278_cu_() + (double)(entityLiving.getRNG().nextInt(16) - 8), 0.0D, (double)(worldIn.func_234938_ad_() - 1));
            double d5 = entityLiving.func_226281_cx_() + (entityLiving.getRNG().nextDouble() - 0.5D) * 16.0D;
