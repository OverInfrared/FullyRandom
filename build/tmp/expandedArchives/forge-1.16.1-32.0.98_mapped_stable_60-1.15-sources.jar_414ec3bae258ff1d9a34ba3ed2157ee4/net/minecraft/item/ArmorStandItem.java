...
public class ArmorStandItem extends Item {
   ...
   public ActionResultType onItemUse(ItemUseContext context) {
      ...
      if (direction == Direction.DOWN) {
         ...
         if (world.func_226669_j_(armorstandentity) && world.getEntitiesWithinAABBExcludingEntity(armorstandentity, armorstandentity.getBoundingBox()).isEmpty()) {
            if (!world.isRemote) {
               float f = (float)MathHelper.floor((MathHelper.wrapDegrees(context.getPlacementYaw() - 180.0F) + 22.5F) / 45.0F) * 45.0F;
               armorstandentity.setLocationAndAngles(armorstandentity.func_226277_ct_(), armorstandentity.func_226278_cu_(), armorstandentity.func_226281_cx_(), f, 0.0F);
               this.applyRandomRotations(armorstandentity, world.rand);
               world.addEntity(armorstandentity);
               world.playSound((PlayerEntity)null, armorstandentity.func_226277_ct_(), armorstandentity.func_226278_cu_(), armorstandentity.func_226281_cx_(), SoundEvents.ENTITY_ARMOR_STAND_PLACE, SoundCategory.BLOCKS, 0.75F, 0.8F);
            }
