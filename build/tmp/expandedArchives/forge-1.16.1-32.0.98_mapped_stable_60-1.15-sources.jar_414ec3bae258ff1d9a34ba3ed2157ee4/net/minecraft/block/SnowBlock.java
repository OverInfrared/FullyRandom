...
public class SnowBlock extends Block {
   ...
   public boolean isValidPosition(BlockState state, IWorldReader worldIn, BlockPos pos) {
      ...
      if (!blockstate.isIn(Blocks.ICE) && !blockstate.isIn(Blocks.PACKED_ICE) && !blockstate.isIn(Blocks.BARRIER)) {
         if (!blockstate.isIn(Blocks.field_226907_mc_) && !blockstate.isIn(Blocks.SOUL_SAND)) {
            return Block.doesSideFillSquare(blockstate.getCollisionShape(worldIn, pos.func_177977_b()), Direction.UP) || blockstate.getBlock() == this && blockstate.get(LAYERS) == 8;
         } else {
            return true;
         }
      } else {
