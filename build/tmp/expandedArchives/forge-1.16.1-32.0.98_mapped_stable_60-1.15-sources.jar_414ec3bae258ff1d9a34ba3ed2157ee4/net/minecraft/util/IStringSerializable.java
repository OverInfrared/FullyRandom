   ...
   static Keyable func_233025_a_(final IStringSerializable[] p_233025_0_) {
      return new Keyable() {
         public <T> Stream<T> keys(DynamicOps<T> p_keys_1_) {
            return p_keys_1_.compressMaps() ? IntStream.range(0, p_233025_0_.length).mapToObj(p_keys_1_::createInt) : Arrays.stream(p_233025_0_).map(IStringSerializable::getName).map(p_keys_1_::createString);
         }
      };
   }
}