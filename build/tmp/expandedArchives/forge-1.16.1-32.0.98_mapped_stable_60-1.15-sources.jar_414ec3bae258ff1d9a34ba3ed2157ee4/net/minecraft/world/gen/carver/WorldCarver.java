...
public abstract class WorldCarver<C extends ICarverConfig> extends net.minecraftforge.registries.ForgeRegistryEntry<WorldCarver<?>> {
   ...
   protected boolean canCarveBlock(BlockState state, BlockState aboveState) {
      return this.func_222706_a(state) || (state.isIn(Blocks.SAND) || state.isIn(Blocks.GRAVEL)) && !aboveState.getFluidState().isTagged(FluidTags.WATER);
   }
      ...
      for(int i = minX; i < maxX; ++i) {
         for(int j = minZ; j < maxZ; ++j) {
            for(int k = minY - 1; k <= maxY + 1; ++k) {
               if (this.carvableFluids.contains(chunkIn.getFluidState(blockpos$mutable.func_181079_c(i + chunkX * 16, k, j + chunkZ * 16)).getFluid())) {
                  return true;
               }
