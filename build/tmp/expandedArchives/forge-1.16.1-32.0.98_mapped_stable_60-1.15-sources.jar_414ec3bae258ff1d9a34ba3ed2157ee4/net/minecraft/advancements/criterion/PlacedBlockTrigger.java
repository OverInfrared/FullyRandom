...
public class PlacedBlockTrigger extends AbstractCriterionTrigger<PlacedBlockTrigger.Instance> {
   ...
   public void trigger(ServerPlayerEntity player, BlockPos pos, ItemStack item) {
      BlockState blockstate = player.func_71121_q().getBlockState(pos);
      this.func_235959_a_(player, (p_226949_4_) -> {
         return p_226949_4_.func_193210_a(blockstate, pos, player.func_71121_q(), item);
      });
   }
   ...
   public static class Instance extends CriterionInstance {
      ...
      public static PlacedBlockTrigger.Instance placedBlock(Block p_203934_0_) {
         return new PlacedBlockTrigger.Instance(EntityPredicate.AndPredicate.field_234582_a_, p_203934_0_, StatePropertiesPredicate.field_227178_a_, LocationPredicate.ANY, ItemPredicate.ANY);
      }

      public boolean func_193210_a(BlockState state, BlockPos pos, ServerWorld world, ItemStack item) {
         if (this.block != null && !state.isIn(this.block)) {
            return false;
         } else if (!this.properties.func_227181_a_(state)) {
            return false;
         } else if (!this.location.func_193453_a(world, (float)pos.getX(), (float)pos.getY(), (float)pos.getZ())) {
            return false;
         } else {
