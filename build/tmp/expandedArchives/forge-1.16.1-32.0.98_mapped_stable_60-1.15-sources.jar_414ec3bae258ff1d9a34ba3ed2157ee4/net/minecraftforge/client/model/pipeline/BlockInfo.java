...
public class BlockInfo
{
    ...
    public void updateLightMatrix()
    {
        for(int x = 0; x <= 2; x++)
        {
            for(int y = 0; y <= 2; y++)
            {
                for(int z = 0; z <= 2; z++)
                {
                    ...
                    int brightness = WorldRenderer.func_228421_a_(world, pos);
                    s[x][y][z] = LightTexture.func_228454_b_(brightness);
                    b[x][y][z] = LightTexture.func_228450_a_(brightness);
        ...
        for(Direction side : SIDES)
        {
            ...
            if(state.getOpacity(world, pos) == 15 || VoxelShapes.func_223416_b(thisStateShape.func_215702_a(world, blockPos, side), otherStateShape.func_215702_a(world, pos, side.getOpposite())))
            {
    ...
    public void updateFlatLighting()
    {
        ...
        packed[0] = WorldRenderer.func_228421_a_(world, blockPos);

        for (Direction side : SIDES)
        {
            int i = side.ordinal() + 1;
            packed[i] = WorldRenderer.func_228421_a_(world, blockPos.offset(side));
        }
    }
