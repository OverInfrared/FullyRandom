...
class CommandEntity
{
    ...
    private static class EntityListCommand
    {
        ...
        static ArgumentBuilder<CommandSource, ?> register()
        {
            return Commands.literal("list")
                ...
                .then(Commands.argument("filter", StringArgumentType.string())
                    ...
                    .executes(ctx -> execute(ctx.getSource(), StringArgumentType.getString(ctx, "filter"), ctx.getSource().func_197023_e().func_234923_W_()))
                )
                .executes(ctx -> execute(ctx.getSource(), "*", ctx.getSource().func_197023_e().func_234923_W_()));
        }
        ...
        private static int execute(CommandSource sender, String filter, RegistryKey<World> dim) throws CommandSyntaxException
        {
            ...
            ServerWorld world = sender.getServer().func_71218_a(dim); //TODO: DimensionManager so we can hotload? DimensionManager.getWorld(sender.getServer(), dim, false, false);
            if (world == null)
